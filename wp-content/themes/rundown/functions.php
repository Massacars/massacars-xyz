...
function rundown_widgets_init() {
	register_sidebar( array(
		'name'          => __( 'Widget Area', 'rundown' ),
		'id'            => 'sidebar-1',
		'description'   => __( 'Add widgets here to appear in your sidebar.', 'rundown' ),
...
add_action( 'widgets_init', 'rundown_widgets_init' );
...
function rundown_scripts_method() {
	...
	wp_enqueue_style( 'rundown-main-style', get_stylesheet_uri() );
...
function rundown_search_form( $form ) {
    ...
    <input type="submit" id="searchsubmit" value="'. esc_attr__('Search', 'rundown') .'" />
    </div>
    </form>';
...
function rundown_excerpt($limit = 51) {
  $define_excerpt = explode(' ', get_the_excerpt(), $limit);
  if (count($define_excerpt) >= $limit) {
	array_pop($define_excerpt);
	$excerpt = implode(' ',$define_excerpt);
  } else {
	$excerpt = implode(' ',$define_excerpt);
  }
...
// Main navigation
add_action('rundown_main_nav', 'rundown_build_main_nav', 5);
if ( ! function_exists( 'rundown_build_main_nav' ) ) :

function rundown_build_main_nav(){
	wp_nav_menu( array(
